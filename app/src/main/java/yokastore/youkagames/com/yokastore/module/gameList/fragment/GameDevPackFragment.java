...
import yokastore.youkagames.com.yokastore.module.gameList.adapter.FragmentGameDevPackageAdapter;
...
public class GameDevPackFragment extends BaseFragment implements IBaseView{
    ...
    private FragmentGameDevPackageAdapter mAdapter;
    ...
    private void initRecycleView() {
        ...
        mAdapter = new FragmentGameDevPackageAdapter(getActivity(),mGameDetailData);

        mAdapter.setClickCallBack(new FragmentGameDevPackageAdapter.ItemClickCallBack() {
            @Override
    ...
    public void initDataFragment() {
        ...
        mPresenter.getGamePackageDatas(mGameDetailData.projectId,"Android","Dev");
        ...
        if (data.code == 0) {
            if (data instanceof GamePackageModel) {
                ...
                if (model.data != null && model.data.size() > 0) {
                    ...
                    mAdapter.updateGameDevPackageData(mGamePackageDatas);
                    if(mRecyclerView != null) {
                        if(model.data.size() < 25){
                            mRecyclerView.getDefaultFootView().setNoMoreHint(getString(R.string.already_loaded_all_data));
                            mRecyclerView.getDefaultFootView().setVisibility(View.VISIBLE);
                        }
                        mRecyclerView.refreshComplete();
                    }
                } else if (mRecyclerView != null){
                    mRecyclerView.getDefaultFootView().setNoMoreHint("no content");
                    //mRecyclerView.setNoMore(true);
                    mRecyclerView.refreshComplete();
                }
            }
