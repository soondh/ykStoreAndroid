...
/**
 * Created by songdehua on 2018/8/7.
 * 游戏安装包列表Adapter
 */

public class FragmentGameDevPackageAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    private ItemClickCallBack clickCallBack;
    private Activity mContext;
    private GameDetailModel.GameDetailData gameDetailData;
    private ArrayList<GamePackageModel.DataBean> gameDevPackageData;

    public void setClickCallBack(ItemClickCallBack clickCallBack) {
        this.clickCallBack = clickCallBack;
    }

    public FragmentGameDevPackageAdapter(Activity context,GameDetailModel.GameDetailData data){
        mContext = context;
        gameDetailData = data;
    }

    public void updateGameDevPackageData(ArrayList<GamePackageModel.DataBean> dataBeans){
        gameDevPackageData  = dataBeans;
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof ViewTypeTwoHolderScore) {
            setGamePackageData(position,(ViewTypeTwoHolderScore) holder);
        }
    }

    private void setGamePackageData(int position, ViewTypeTwoHolderScore holder){
        if (gameDevPackageData != null && gameDevPackageData.size() > 0){
            GamePackageModel.DataBean dataBean = gameDevPackageData.get(position);
